...
const leadForm = await readFile(resolve(root, 'media/lead-form.js'), 'utf8');
const formRegistrationMigration = await readFile(resolve(root, 'supabase/migrations/20260802045805_register_vgg_marketing_forms.sql'), 'utf8');
const formConfigSource = await readFile(resolve(root, 'netlify/functions/vgg-form-config.js'), 'utf8');
const intakeSource = await readFile(resolve(root, 'netlify/functions/vgg-lead-intake.js'), 'utf8');
...
assert.match(app, /function renderFormSubmissions/);
assert.match(app, /data-edit-form/);
assert.match(app, /data-copy-form/);
assert.match(app, /ATRIBUCIÓN DEL LEAD/);
assert.match(html, /id="form-submission-list"/);
...
assert.match(embed, /vgg:form-submitted/);
assert.match(embed, /vgg:form-ready/);
assert.match(embed, /vgg:form-error/);
assert.match(embed, /privacy_url/);
assert.match(leadForm, /(?:const|var) FORM_KEYS/);
assert.match(leadForm, /data-vgg-embed-runtime/);
...
assert.doesNotMatch(formRegistrationMigration, /amitai/i);
const registeredFieldConfigs = [...formRegistrationMigration.matchAll(/'(\[[^\n]+\])'::jsonb/g)].map((match) => JSON.parse(match[1]));
assert.equal(registeredFieldConfigs.length, 7);
assert.ok(registeredFieldConfigs.some((fields) => fields.some((field) => field.name === 'project_type')));
assert.ok(registeredFieldConfigs.some((fields) => fields.some((field) => field.name === 'start_window')));
assert.ok(registeredFieldConfigs.some((fields) => fields.some((field) => field.name === 'event_date' && field.type === 'date')));
...
assert.match(intakeSource, /submittedPage && !domainAllowed/);
assert.match(intakeSource, /Tipo de proyecto:/);
assert.match(intakeSource, /Ventana de inicio:/);
assert.match(intakeSource, /Fecha tentativa:/);
